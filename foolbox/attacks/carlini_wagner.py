    ...
    def run(
        ...
        **kwargs: Any,
        ...
        def loss_fun(
            ...
            """
            similarity = cos_similarity_score(logits.raw,classes.raw)
            threshold = torch.Tensor([self.threshold]*self.totalsize).to(self.device)
            is_adv_loss = -torch.log(torch.minimum(similarity, threshold)/(self.threshold))
            assert is_adv_loss.shape == (N,)

            is_adv_loss = is_adv_loss + self.confidence
            is_adv_loss = ep.maximum(torch.Tensor([0]).to(self.device), is_adv_loss)
        ...
        for binary_search_step in range(self.binary_search_steps):
            print(f"binary search step: {binary_search_step+1}")
            """
            if (
            ...
            for step in range(self.steps):
                ...
                if self.abort_early and step>=100 and step % 10 == 0:
                ...
                found_advs_iter = cos_similarity_score(logits.raw, classes.raw)>self.threshold
                found_advs = np.logical_or(found_advs, found_advs_iter.cpu().numpy())
                ...
                del loss, perturbed, logits, found_advs_iter
            del delta
