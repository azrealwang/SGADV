...
class DeepFoolAttack(MinimizationAttack, ABC):
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        self.totalsize = classes.shape[0]
        ...
        for _ in range(self.steps):
            ...
            diffs = [loss_aux_and_grad(x)]
            _, (_, logits), _ = diffs[0]
            is_adv = cos_similarity_score(logits.raw, classes.raw)>self.threshold
            if is_adv.all():
                break
            ...
            assert losses.shape == (N, candidates)
            assert grads.shape == (N, candidates) + x0.shape[1:]

            # calculate the distances
            distances = self.get_distances(losses, grads)
            assert distances.shape == (N, candidates)
            ...
            p_total += p_step
            # don't do anything for those that are already adversarials
            condition = ep.astensor(atleast_kd(is_adv, x.ndim)).bool()
            x = ep.where(
                condition, x, x0 + (1.0 + self.overshoot) * p_total
            )
